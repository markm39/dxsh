...
class WorkflowClient:
    ...
    async def get_workflow_agent(self, agent_id: int) -> Optional[Dict]:
        """Get workflow agent details"""
        try:
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                response = await client.get(f"{self.base_url}/v1/workflows/{agent_id}")
    ...
    async def get_user_workflow_agents(self, user_id: int) -> List[Dict]:
    ...
    async def get_node_execution_data(self, agent_id: int, node_id: str) -> Optional[Any]:
