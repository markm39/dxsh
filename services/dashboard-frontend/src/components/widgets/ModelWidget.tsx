...
const ModelWidget: React.FC<ModelWidgetProps> = ({
  ...
  const modelData = React.useMemo(() => {
    ...
    console.log('Model widget received data:', {
      ...
      predictionsSample: data?.predictions?.[0],
      data: data
    });
    ...
    if (data.success && data.model) {
      ...
      processedData = data;
    } else if (data.model_id && data.model_type && data.model_name) {
      // ML node execution data format
      console.log('Found ML node execution data format');
      processedData = data;
    }
    ...
    const transformedData = {
      id: processedData.model_id || processedData.id,
      ...
      schema: {
        features: (processedData.features || processedData.feature_names) ? 
          (processedData.features || processedData.feature_names).map((name: string, index: number) => ({
            name,
        ...
        target: {
          name: processedData.target || processedData.target_name || 'target',
          type: 'number',
          displayName: processedData.target || processedData.target_name || 'Target',
        }
      },
      performance: {
        score: processedData.metrics?.r2_score || processedData.training_metrics?.train_r2 || processedData.training_metrics?.test_r2 || processedData.training_metrics?.r2_score || 0,
        r_squared: processedData.metrics?.r2_score || processedData.training_metrics?.train_r2 || processedData.training_metrics?.r2_score,
        mean_squared_error: processedData.metrics?.mse || processedData.training_metrics?.train_mse || processedData.training_metrics?.test_mse || processedData.training_metrics?.mse,
        mean_absolute_error: processedData.metrics?.mae || processedData.training_metrics?.train_mae || processedData.training_metrics?.test_mae || processedData.training_metrics?.mae,
        accuracy: processedData.metrics?.accuracy || processedData.training_metrics?.accuracy,
        feature_importance: (() => {
          ...
          } else if (processedData.feature_importance && processedData.features) {
            // ML node format: feature_importance array + features array
            const features = processedData.features;
            const importance = processedData.feature_importance;
        ...
        training_info: {
          ...
          dataset_size: processedData.training_samples || processedData.n_samples || 0,
          test_samples: processedData.test_samples || 0
        }
      },
      model_data: {
      ...
      visualizations: (() => {
        ...
        if (data.visualization_data) {
          // Original visualization data format
          if (data.visualization_data.actual_vs_predicted) {
            visualizations.push({
          ...
          if (data.visualization_data.residuals) {
            visualizations.push({
              id: 'residuals',
              type: 'scatter',
              title: 'Residuals',
        ...
        // Check for predictions data in multiple locations
        let predictions = null;
        if (processedData.predictions && Array.isArray(processedData.predictions) && processedData.predictions.length > 0) {
        ...
        if (predictions) {
          // ML node format with predictions array
          console.log('Creating predictions visualization from:', predictions);
          console.log('Sample prediction structure:', predictions[0]);
          
          // Transform to the format expected by ModelVisualization component
          const actualValues = predictions.map((pred: any) => pred.actual);
          const predictedValues = predictions.map((pred: any) => pred.predicted);
          const residualValues = predictions.map((pred: any) => pred.actual - pred.predicted);
        ...
        // Add feature importance visualization for ML node format
        if (processedData.feature_importance && processedData.features) {
          const features = processedData.features;
          const importance = processedData.feature_importance;
          console.log('Creating feature importance visualization from:', { features, importance });
          visualizations.push({
            ...
            data: features.map((feature: string, index: number) => ({
              feature,
              importance: importance[index] || 0,
              coefficient: importance[index] || 0,
              x: feature,
              y: importance[index] || 0
            }))
          });
        }
    ...
    return transformedData as unknown as ModelData;
  }, [data]);
  ...
  const handlePredict = React.useCallback(async (inputs: { [feature: string]: any }): Promise<PredictionResult> => {
    ...
    try {
      // Make prediction API call through API Gateway
