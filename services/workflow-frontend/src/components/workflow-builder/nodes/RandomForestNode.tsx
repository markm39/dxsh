...
import { TreePine, Loader2, CheckCircle, AlertCircle, Brain, Play, Trash2 } from 'lucide-react';

interface RandomForestNodeProps {
  data: {
    label: string;
    configured: boolean;
    model?: any;
...
const RandomForestNode: React.FC<RandomForestNodeProps> = ({ data, onRunFromHere, onDelete }) => {
  const { 
    label, 
    configured = false, 
    model,
    ...
    switch (status) {
      ...
      case 'configured':
        return {
          ...
          borderColor: "border-green-500/30",
          nodeBorder: "border-green-500/50 shadow-green-500/20",
          icon: <Brain className="w-3 h-3 text-green-400" />
        };
      default:
        return {
    ...
    <div className={`bg-background border-2 rounded-lg p-4 min-w-[200px] max-w-[250px] shadow-sm hover:shadow-md transition-all duration-200 ${styles.nodeBorder}`}>
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className={`w-8 h-8 bg-green-500/20 border border-green-500/30 rounded-lg flex items-center justify-center transition-all duration-200 ${isTraining || isExecuting ? 'animate-pulse bg-green-500/30' : ''}`}>
        ...
        <div className="flex gap-1">
          ...
          {onDelete && (
            <button
              ...
              disabled={isTraining || isExecuting}
              className={`p-1.5 rounded-md transition-colors ${
                isTraining || isExecuting
                  ? "opacity-50 cursor-not-allowed text-text-muted"
      ...
      {configured && actualModelConfig && (
        <div className="text-xs text-text-muted mb-2">
          ...
          {actualModelConfig.nEstimators && (
            <div>Trees: {actualModelConfig.nEstimators}</div>
          )}
          {actualModelConfig.maxDepth && (
            <div>Max Depth: {actualModelConfig.maxDepth}</div>
          )}
      ...
      {(executionResult || trainingResult) && (
        <div className="mb-3 p-2 bg-surface-secondary/30 rounded border border-border-subtle/50">
          ...
          <div className="text-xs text-text-muted bg-background/50 p-1.5 rounded border border-border-subtle/30 space-y-1">
            ...
            {(executionResult || trainingResult) && (
              <div className="text-xs text-text-muted">Random Forest trained and ready</div>
            )}
          </div>
        </div>
      )}
      ...
      {!configured && (
        <div className="text-xs text-text-muted italic">
          Click to configure Random Forest training
        </div>
      )}
    </div>
  );
