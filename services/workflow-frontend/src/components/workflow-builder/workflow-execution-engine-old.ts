...
export class WorkflowExecutionEngine {
  ...
  private async executeWebSource(nodeId: string, nodeData: WorkflowNodeData, inputs: NodeOutput[]): Promise<any> {
    ...
    if (selectors.length > 0 && selectors[0].selector && !selectors[0].id) {
      // Already in processed format, use directly but ensure type field is set
      console.log('✅ USING PRE-PROCESSED FORMAT: selectors have .selector but no .id');
      selectorsPayload = selectors.map((sel: any) => ({
        ...sel,
        type: sel.type || 'all', // Ensure type field is always set, default to 'all'
        attribute: sel.type === 'table' ? 'table_data' : (sel.attribute || 'textContent') // Set correct attribute for table types
      }));
    } else {
      ...
      if (repeatingSelector && repeatingSelector.fields) {
        ...
        selectorsPayload = selectors.map((sel: any) => ({
          ...
          label: sel.label,
          name: sel.name,
          attribute: sel.type === 'table' ? 'table_data' : (sel.attribute === "all" ? "all" : (sel.attribute === "table_data" ? "table_data" : "textContent")),
          type: sel.type || 'all' // Ensure type field is always set, default to 'all'
        }));
      }
    }
